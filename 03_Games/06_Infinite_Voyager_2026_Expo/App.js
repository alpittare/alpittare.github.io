...
const SUPERWALL_API_KEYS = {
  ios: 'pk_YOUR_SUPERWALL_KEY_HERE',
};

const BG_COLOR = '#001a2e';
const BRIDGE_TAG = 'InfiniteVoyager';
  ...
  const onMessage = useCallback(async (event) => {
    try {
      ...
      if (data.type === 'restore_purchases') {
        if (webViewRef.current) {
          webViewRef.current.postMessage(JSON.stringify({ type: 'restore_complete' }));
        }
      }
    ...
    (function() {
      ...
      window.NativePurchase = {
        buy: function(productId) {
          window.ReactNativeWebView.postMessage(JSON.stringify({
            type: 'purchase', productId: productId
          }));
        },
        restore: function() {
          window.ReactNativeWebView.postMessage(JSON.stringify({
      ...
      document.addEventListener('message', function(e) {
        try {
          ...
          if (data.type === 'purchase_success') window.dispatchEvent(new CustomEvent('purchaseSuccess', { detail: data }));
          if (data.type === 'products_list') window.dispatchEvent(new CustomEvent('productsLoaded', { detail: data }));
          if (data.type === 'restore_complete') window.dispatchEvent(new CustomEvent('restoreComplete', { detail: data }));
        } catch(ex) {}
      });
